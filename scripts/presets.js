...
export default function registerPresets() {
    return [...game.modules.values()].filter(m => m.active && m.data.flags["custom-fonts"]).map(p => new CustomFontsPreset(p));
}
    ...
    async load(preset) {
        try {
            ...
            if (fonts) {
                ...
                await game.settings.set(CustomFonts.ID, "fonts", [
                    existing,
                    this.findNewFonts(fonts, existing)
                ].filter(x => x.length).join(", "));
            }

            if (directory) {
                const existing = game.settings.get(CustomFonts.ID, "presetDirectories");
                if (!existing.includes(this.path + directory)) game.settings.set(CustomFonts.ID, "presetDirectories", [
                    ...existing,
                    this.path + directory
                ])
            ...
            ui.notifications.info(`${CustomFonts.ID} | ${game.i18n.format("custom-fonts.notifications.presetLoaded", { preset: this.title })}`);
        } catch (err) {
            doOnceReady(() => {
                const message = `${CustomFonts.ID} | ${game.i18n.format("custom-fonts.notifications.invalidPreset", { preset: this.title, error: err })}`;
